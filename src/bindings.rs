...
pub mod quadrature {
    ...
    pub unsafe extern "C" fn gauss_jacobi_quadrature_npoints(cell: u8, m: usize) -> usize {
        quadrature::gauss_jacobi_quadrature_npoints(ReferenceCellType::from(cell).expect("Invalid cell type"), m)
    }
    unsafe fn make_gauss_jacobi_quadrature<T: RlstScalar<Real = T> + FloatConst + PartialOrd>(cell: u8, m: usize, pts: *mut T, wts: *mut T) {
        let rule = quadrature::make_gauss_jacobi_quadrature::<T>(ReferenceCellType::from(cell).expect("Invalid cell type"), m);
    ...
    #[no_mangle]
    pub unsafe extern "C" fn make_gauss_jacobi_quadrature_f32(cell: u8, m: usize, pts: *mut f32, wts: *mut f32) {
        make_gauss_jacobi_quadrature(cell, m, pts, wts);
    }
    #[no_mangle]
    pub unsafe extern "C" fn make_gauss_jacobi_quadrature_f64(cell: u8, m: usize, pts: *mut f64, wts: *mut f64) {
        make_gauss_jacobi_quadrature(cell, m, pts, wts);
    }
}
...
pub mod polynomials {
    ...
    use crate::types::ReferenceCellType;
    use rlst::{RlstScalar, rlst_array_from_slice2, rlst_array_from_slice_mut3};
    use std::slice::{from_raw_parts, from_raw_parts_mut};

    #[no_mangle]
    pub unsafe extern "C" fn legendre_polynomials_shape(cell: u8, npts: usize, degree: usize, derivatives: usize, shape: *mut usize) {
    ...
    unsafe fn tabulate_legendre_polynomials<T: RlstScalar>(cell: u8, points: *const T::Real, npts: usize, degree: usize, derivatives: usize, data: *mut T) {
        let cell_type = ReferenceCellType::from(cell).expect("Invalid cell type");
        ...
        let mut data = rlst_array_from_slice_mut3!(from_raw_parts_mut(data, npts * npoly * nderiv), [nderiv, npoly, npts]);
        polynomials::tabulate_legendre_polynomials(cell_type, &points, degree, derivatives, &mut data);
    }
    #[no_mangle]
    pub unsafe extern "C" fn tabulate_legendre_polynomials_f32(cell: u8, points: *const f32, npts: usize, degree: usize, derivatives: usize, data: *mut f32) {
        tabulate_legendre_polynomials(cell, points, npts, degree, derivatives, data);
    }
    #[no_mangle]
    pub unsafe extern "C" fn tabulate_legendre_polynomials_f64(cell: u8, points: *const f64, npts: usize, degree: usize, derivatives: usize, data: *mut f64) {
        tabulate_legendre_polynomials(cell, points, npts, degree, derivatives, data);
    }
