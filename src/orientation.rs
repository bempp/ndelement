...
use crate::{types::ReferenceCellType, reference_cell};
use itertools::izip;
...
pub fn compute_orientation(entity_type: ReferenceCellType, vertices: &[usize]) -> i32 {
    ...
    if dim > 2 {
        for (t, v) in izip!(&reference_cell::entity_types(entity_type)[2], reference_cell::faces(entity_type)) {
            println!("{t:?} {v:?}");
            match t {
                ReferenceCellType::Triangle => {
                    let m = v.iter().map(|i| vertices[*i]).enumerate()
                        .min_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap())
                        .map(|(index, _)| index).unwrap() as i32;
                    orientation += m << n;
                    n += 2;
                ...
                _ => { panic!("Unsupported face type: {t:?}"); }
            }
        }
    }
